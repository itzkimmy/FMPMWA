...
export function getPaidCents(booking: { transactions: Transaction[] }): number {
...
export function isOnPaymentWatch(booking: BookingWithTransactions): boolean {
  ...
  const depositUnpaid = booking.depositCents > 0 ? paidCents < booking.depositCents : paidCents === 0;
