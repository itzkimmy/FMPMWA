...
async function testHttpEndpoints() {
  ...
  assert.ok(cookieHeader && cookieHeader.includes("flowmotion_session="));
  const sessionCookie = cookieHeader.split(";")[0];
  console.log("✔ Login successful and FlowMotion session cookie issued!");
  ...
  for (const route of routes) {
    ...
    assert.ok(html.includes("FlowMotion"), `Route ${route} rendered FlowMotion layout`);
